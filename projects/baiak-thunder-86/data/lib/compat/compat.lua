-- Função table.contains necessária para createFunctions
...
do
	local function ActionNewIndex(self, key, value)
		if key == "onUse" then
			if self.onUse then self:onUse(value) end
			return
		end
...
do
	local function TalkActionNewIndex(self, key, value)
		if key == "onSay" then
			if self.onSay then self:onSay(value) end
			return
		end
		if type(self) == "table" then
			rawset(self, key, value)
...
do
	local function CreatureEventNewIndex(self, key, value)
		local types = {
			onLogin = "login", onLogout = "logout", onThink = "think",
			onPrepareDeath = "preparedeath", onDeath = "death", onKill = "kill",
			onAdvance = "advance", onModalWindow = "modalwindow", onTextEdit = "textedit",
			onHealthChange = "healthchange", onManaChange = "manachange",
			onExtendedOpcode = "extendedopcode"
		}

		local typeStr = types[key]
		if typeStr then
			if self.type then self:type(typeStr) end
			if self[key] then self[key](self, value) end
			return
		end
...
do
	local function GlobalEventNewIndex(self, key, value)
		local types = {
			onThink = "think", onTime = "time",
			onStartup = "startup", onShutdown = "shutdown", onRecord = "record"
		}
...
do
	local function WeaponNewIndex(self, key, value)
		if key == "onUseWeapon" then
			if self.onUseWeapon then self:onUseWeapon(value) end
			return
		end
...
do
	local function SpellNewIndex(self, key, value)
		if key == "onCastSpell" then
			if self.onCastSpell then self:onCastSpell(value) end
			return
		end
...
do
	local function MonsterTypeNewIndex(self, key, value)
		local types = {
			onThink = MONSTERS_EVENT_THINK, onAppear = MONSTERS_EVENT_APPEAR,
			onDisappear = MONSTERS_EVENT_DISAPPEAR, onMove = MONSTERS_EVENT_MOVE,
			onSay = MONSTERS_EVENT_SAY
		}

		local typeConst = types[key]
		if typeConst then
			if self.eventType then self:eventType(typeConst) end
			if self[key] then self[key](self, value) end
			return
		end
