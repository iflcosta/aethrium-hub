...
function getTopFraggers(vocs)
    ...
    repeat
        ...
        if vocs then
            local resultId = db.storeQuery("SELECT `vocation` FROM `players` WHERE `name` = " .. db.escapeString(v[2]) .. " LIMIT 1")
...
function rank.onSay(player, words, param)
    if player:getStorageValue(exhaustvalue) >= os.time() then
        player:sendTextMessage(errorcolor, "Please wait a few seconds.")
        return false
    end
    ...
    if ranks[split[1]] then
        local msg = "Top players, " .. stats_names[ranks[split[1]]][#stats_names[ranks[split[1]]]] .. (voc[split[2]] and (", " .. split[2]) or "") .. ":"
    ...
    player:sendTextMessage(errorcolor, "Incorrect list name. Execute the command without parameters to see available lists.")
    return false
end
