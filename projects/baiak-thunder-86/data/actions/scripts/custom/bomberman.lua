...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if #players == 10 then
        for i = 1, 10 do
            if not players[i]:removeMoney(100000) then
                player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "At least one player (item " .. i .. ") does not have the 100,000 gold (100k) required to play.")
                player:getPosition():sendMagicEffect(CONST_ME_POFF)
                return true
            end
        end
        ...
        for i = 1, 10 do
            ...
            if players[i]:isPlayer() then
                players[i]:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
                if i <= 5 then
            ...
            elseif players[i]:isMonster() then
                players[i]:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
            end
...
function endGame()
    ...
    for i = 1, #BomberTeam1 do
        if BomberTeam1[i]:isPlayer() then
    ...
    for i = 1, #BomberTeam2 do
        if BomberTeam2[i]:isPlayer() then
