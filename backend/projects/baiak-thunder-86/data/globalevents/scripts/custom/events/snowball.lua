function startGame(rounds)
	if rounds == 0 then
		if #CACHE_GAMEPLAYERS < SNOWBALL.minPlayers then
			...
			broadcastMessage(SNOWBALL.prefix .. SNOWBALL.mensagemEventoFaltaPlayer:format(SNOWBALL.minPlayers))
		else
			for _, players in ipairs(CACHE_GAMEPLAYERS) do
			...
			broadcastMessage(SNOWBALL.prefix .. SNOWBALL.mensagemInicioEvento)
			addEvent(terminarEvento, SNOWBALL.duracaoEvento * 60 * 1000)
		end

		local tileItem = getTileItemById(SNOWBALL.posTpEntrarEvento, 1387)
		if tileItem and tileItem.uid > 0 then
			Item(tileItem.uid):remove(1)
		end
		return true
	end

	if #CACHE_GAMEPLAYERS < SNOWBALL.minPlayers then
		broadcastMessage(SNOWBALL.prefix .. (SNOWBALL.mensagemEsperandoIniciar):format(rounds, SNOWBALL.minPlayers - #CACHE_GAMEPLAYERS))
	else
		broadcastMessage(SNOWBALL.prefix .. (SNOWBALL.mensagemEsperandoIniciar):format(rounds))
	end
...
function onThink(interval)
	if SNOWBALL.days[os.date("%A")] then
		...
		if isInArray(SNOWBALL.days[os.date("%A")], hrs) then
			CACHE_GAMEPLAYERS = {}
			broadcastMessage(SNOWBALL.prefix .. SNOWBALL.mensagemEventoAberto)
