...
-- outfitsdoll
dofile('data/lib/modalhelper/outfitsdoll.lua')
...
function Player:getMarriageDescription(thing)
    ...
    if getPlayerMarriageStatus(thing:getGuid()) == MARRIED_STATUS then
        local playerSpouse = getPlayerSpouse(thing:getGuid())
        if self == thing then
            descr = descr .. " Você é casado"
        elseif thing:getSex() == PLAYERSEX_FEMALE then
            descr = descr .. " Ela é casada"
        else
            descr = descr .. " Ele é casado"
        end
        descr = descr .. " com " .. getPlayerNameById(playerSpouse) .. '.'
    end
    return descr
end
...
function Game.guildLeaderSquare(playerId)
	...
	-- Only process if player is real and a guild leader (level 3)
	if player:getGuildLevel() == 3 then
		local pos = player:getPosition()
		local spectators = Game.getSpectators(pos, true, true, 7, 7, 5, 5)
		local pid = player:getId()
		for i = 1, #spectators do
			spectators[i]:sendCreatureSquare(pid, 215)
		end
		addEvent(Game.guildLeaderSquare, 1000, pid) -- 1s is enough for visual sync
	else
		-- If not leader, check again in 5 seconds (in case they become leader or for lazy cleanup)
		addEvent(Game.guildLeaderSquare, 5000, playerId)
	end
end