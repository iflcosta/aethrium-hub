...
function Player.transferMoneyTo(self, target, amount)
	...
	if type(target) == "string" then
		targetGuid = getPlayerGUIDByName(target)
		if not targetGuid then return false end
	elseif type(target) == "userdata" and target.isPlayer and target:isPlayer() then
		targetGuid = target:getGuid()
	elseif type(target) == "table" and target.guid then
	...
	local targetPlayer = Player(targetGuid)
...
function Player.getMoney(self)
	return self:getMoneyTotal()
end

function Player.getMoneyTotal(self)
	local gold = self:getItemCount(2148)
	gold = gold + (self:getItemCount(2152) * 100)
	gold = gold + (self:getItemCount(2160) * 10000)
	gold = gold + (self:getItemCount(15515) * 1000000)
	return gold
end

function Player.removeMoney(self, amount, flags, showmsg)
	return self:removeMoneyTotal(amount)
end

function Player.removeMoneyTotal(self, amount)
	local total = self:getMoneyTotal()
	...
	-- 1. Step-by-step consumption from lowest to highest
	-- Gold (1)
	local count = self:getItemCount(2148)
	local toRemove = math.min(remaining, count)
	if toRemove > 0 then
		self:removeItem(2148, toRemove)
		remaining = remaining - toRemove
	end

	-- Platinum (100)
	if remaining > 0 then
		count = self:getItemCount(2152)
		toRemove = math.min(math.floor(remaining / 100), count)
		if toRemove > 0 then
			self:removeItem(2152, toRemove)
			remaining = remaining - (toRemove * 100)
		end
	end

	-- Crystal (10,000)
	if remaining > 0 then
		count = self:getItemCount(2160)
		toRemove = math.min(math.floor(remaining / 10000), count)
		if toRemove > 0 then
			self:removeItem(2160, toRemove)
			remaining = remaining - (toRemove * 10000)
		end
	end

	-- Bar of Gold (1,000,000)
	if remaining > 0 then
		count = self:getItemCount(15515)
		toRemove = math.min(math.floor(remaining / 1000000), count)
		if toRemove > 0 then
			self:removeItem(15515, toRemove)
			remaining = remaining - (toRemove * 1000000)
		end
	end

	-- 2. If STILL remaining, we need to break the next available higher coin
	if remaining > 0 then
		-- Try breaking Platinum for Gold change
		if remaining < 100 and self:removeItem(2152, 1) then
			self:addItem(2148, 100 - remaining)
			remaining = 0
		-- Try breaking Crystal for change (efficiently)
		elseif remaining < 10000 and self:removeItem(2160, 1) then
			self:addMoney(10000 - remaining)
			remaining = 0
		-- Try breaking Bar for change (efficiently)
		elseif remaining < 1000000 and self:removeItem(15515, 1) then
			self:addMoney(1000000 - remaining)
			remaining = 0
		end
	end
...
function Player.removeTotalMoney(self, amount)
	local moneyCount = self:getMoneyTotal()
	local bankCount = self:getBankBalance()
	if amount <= moneyCount then
		self:removeMoneyTotal(amount)
		return true
	elseif amount <= (moneyCount + bankCount) then
		if moneyCount ~= 0 then
			self:removeMoneyTotal(moneyCount)
