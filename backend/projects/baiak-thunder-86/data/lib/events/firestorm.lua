...
if RELOAD_LIB_ON or not FSE then
    ...
    --@Timers in seconds
    FSE.timer = {}
    FSE.timer.removeTp = 5 -- in minutes
    ...
    FSE.msg = {
        prefix = "[FireStorm] ",
        openTp = "The event has been opened and awaits players. You have %d minute(s) to enter.",
        forceStop = "The event has been forced to stop.",
        noPlayers = "The event could not start due to lack of players.",
        playerRemoved = "The player %s has been hit.",
        eventFinish = "The event has ended and the winning player was %s.",
        eventFinishNoWin = "The event has ended without players.",
    }
    ...
    function FSE:Stopped(players, causeMessage, forceStopped)
        FSE.status = "Stopped"
    ...
    function FSE:CheckControl()
        ...
        elseif FSE.status == "Waiting" then
            ...
            if #players < FSE.players.min then
                FSE:Stopped(players, FSE.msg.prefix .. FSE.msg.noPlayers)
            else
                FSE:Started()
            end
        elseif FSE.status == "Started" then
            ...
            if #players <= FSE.players.win then
                if #players == 0 then
                    FSE:Stopped(players, FSE.msg.prefix .. FSE.msg.eventFinishNoWin)
                else
                    ...
                    FSE:Stopped(players, FSE.msg.prefix .. string.format(FSE.msg.eventFinish, getWinNames(players)))
                end
            else
    ...
    local function getRewardNames(items)
        ...
        for index, item in pairs(items) do
            ...
            names = names .. item.count .. " " .. it:getName() .. (next(items, index) == nil and '.' or ', ')
        end
        return names
    end
    ...
    function FSE:AttackSignal()
        ...
        if not foundTile then
            return DEBUG_ON and print("No tile could be found in the Firestorm Event area.")
        end
